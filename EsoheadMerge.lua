function EsoheadMerge.importFromEsohead()
	if not EH then
	...
	local profession
	local newMapName
	if not EsoheadMerge.nodes.oldData then
		EsoheadMerge.nodes.oldData = {}
	end
	...
	for map, data in pairs(EH.savedVars["harvest"].data) do
		...
		newMapName = EsoheadMerge.GetNewMapName(map)
		if newMapName then
			for index, nodes in pairs(data) do
				for _, node in pairs(nodes) do
					EsoheadMerge.saveData( newMapName, node[1], node[2], EsoheadMerge.GetProfessionType(node[5], node[4]), node[4], node[5] )
				end
			end
		end
	end
	...
	for map, nodes in pairs(EH.savedVars["chest"].data) do
		...
		if newMapName then
			for _, node in pairs(nodes) do
				...
				EsoheadMerge.saveData( newMapName, node[1], node[2], 6, "chest", nil )
			end
		end
	end
	...
	for map, nodes in pairs(EH.savedVars["fish"].data) do
		...
		if newMapName then
			for _, node in pairs(nodes) do
				...
				EsoheadMerge.saveData( newMapName, node[1], node[2], 9, "fish", nil )
			end
		end
	end
		...
		if newMapName then
			for title, positions in pairs(nodes) do
				for _, coords in pairs(positions) do
					...
					EsoheadMerge.saveData( newMapName, coords[1], coords[2], 10, title, nil )
				end
			end
		end
	end
		...
		if newMapName then
			for itemId, locations in pairs(data[5]) do
				for _, location in pairs(locations) do
					...
					if EsoheadMerge.IsValidContainer(location[4]) then
						EsoheadMerge.saveData( newMapName, location[1], location[2], 8, location[4], itemId )
					end
				end
			end
		end
	...
	d("Finished.")
	EsoheadMerge.RefreshPins()
end

SLASH_COMMANDS["/esoheadmerge"] = EsoheadMerge.importFromEsohead